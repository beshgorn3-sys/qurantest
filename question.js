...
function generateScrambledWords(pageAyahs, qari, handleResultCallback, difficulty) {
    ...
    // *** إصلاح: تم نقل هذا الجزء قبل return ***
...
function generateOrderSequence(pageAyahs, qari, handleResultCallback, optionsCount, isAudio) {
    ...
    // *** إصلاح: إعداد قائمة انتظار للصوت بدلاً من إنشاء وسوم <audio> ***
    let audioQueueForThisQuestion = [];
    if (isAudio) {
        audioQueueForThisQuestion = correctSequence.map(ayah => `https://cdn.islamic.network/quran/audio/128/${qari}/${ayah.number}.mp3`);
    }

    const questionHTML = `
        <h3>السؤال: ${isAudio ? 'استمع للآيات ثم رتبها حسب تسلسلها الصحيح في المصحف.' : 'رتب الآيات التالية حسب تسلسلها الصحيح في المصحف.'}</h3>
        ${isAudio ? `` : ``} // تم إزالة وسوم <audio> من هنا
    ...
    // *** تعديل هنا: إرجاع قائمة الانتظار مع باقي البيانات ***
    return { questionHTML, setupListeners, audioQueue: audioQueueForThisQuestion };
}
